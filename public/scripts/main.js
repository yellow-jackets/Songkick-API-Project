...
var skRef = firebase.database().ref('/UserConcertList');
...
sk.locationEvent = function () {
  $('form').on('submit', function (e) {
    e.preventDefault();
    var userCity = $('.usersLocation').val();
    var userCityLowerCase = userCity.toLowerCase();
    var userCityCapitalized = userCityLowerCase.charAt(0).toUpperCase() + userCityLowerCase.substr(1);
...
sk.filterList = function (concertList) {
  var drilledToConcertArray = concertList.resultsPage.results.event;
...
sk.getArtistsImages = function (concertList) {
  ...
  (_$ = $).when.apply(_$, _toConsumableArray(artistImages)).done(function () {
